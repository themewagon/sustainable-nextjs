...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    <div
      className={`relative ${
        item.label === "Faqs" ? "xl:block lg:hidden block" : null
      }`}
    ...
    >
      <Link
        href={item.href}
        className={`py-3 text-base flex font-normal text-white dark:text-white hover:text-LightApricot dark:hover:text-LightApricot ${
          path === item.href ? "text-LightApricot dark:text-LightApricot" : ""
        } ${
